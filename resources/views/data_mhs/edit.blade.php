...
@section('breadcrumb')
    ...
    <li class="text-blue-500 font-medium">Edit Data Mahasiswa</li>
@endsection
...
<div class="container mx-auto px-4 pb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden w-full">
        {{-- Route diarahkan ke update dengan menyertakan ID Mahasiswa --}}
        <form action="{{ route('data_mhs.update', $mahasiswa->id_mahasiswa) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center gap-3 px-6 pt-5 pb-1">
                ...
                <h3 class="font-bold text-gray-800 uppercase tracking-wider text-sm">Edit Data Mahasiswa</h3>
            </div>
            ...
            <div class="px-6 py-4">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="kelas" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition text-sm text-gray-700 @error('kelas') border-red-500 @enderror">
                            <option value="">-- Pilih Kelas --</option>
                                                        ...
                                                        {{-- TINGKAT 1 --}}
                            <option value="1A" {{ old('kelas', $mahasiswa->kelas) == '1A' ? 'selected' : '' }}>1A</option>
                            <option value="1B" {{ old('kelas', $mahasiswa->kelas) == '1B' ? 'selected' : '' }}>1B</option>
                            <option value="1C" {{ old('kelas', $mahasiswa->kelas) == '1C' ? 'selected' : '' }}>1C</option>
                            <option value="1D" {{ old('kelas', $mahasiswa->kelas) == '1D' ? 'selected' : '' }}>1D</option>

                            {{-- TINGKAT 2 --}}
                            <option value="2A" {{ old('kelas', $mahasiswa->kelas) == '2A' ? 'selected' : '' }}>2A</option>
                            <option value="2B" {{ old('kelas', $mahasiswa->kelas) == '2B' ? 'selected' : '' }}>2B</option>
                            <option value="2C" {{ old('kelas', $mahasiswa->kelas) == '2C' ? 'selected' : '' }}>2C</option>
                            <option value="2D" {{ old('kelas', $mahasiswa->kelas) == '2D' ? 'selected' : '' }}>2D</option>

                            {{-- TINGKAT 3 --}}
                            <option value="3A" {{ old('kelas', $mahasiswa->kelas) == '3A' ? 'selected' : '' }}>3A</option>
                            <option value="3B" {{ old('kelas', $mahasiswa->kelas) == '3b' ? 'selected' : '' }}>3B</option>
                            <option value="3C" {{ old('kelas', $mahasiswa->kelas) == '3C' ? 'selected' : '' }}>3C</option>
                            <option value="3D" {{ old('kelas', $mahasiswa->kelas) == '3D' ? 'selected' : '' }}>3D</option>

                            {{-- TINGKAT 4 --}}
                            <option value="3A" {{ old('kelas', $mahasiswa->kelas) == '4A' ? 'selected' : '' }}>4A</option>
                            <option value="3B" {{ old('kelas', $mahasiswa->kelas) == '4B' ? 'selected' : '' }}>4B</option>
                            <option value="3C" {{ old('kelas', $mahasiswa->kelas) == '4C' ? 'selected' : '' }}>4C</option>
                            <option value="3D" {{ old('kelas', $mahasiswa->kelas) == '4D' ? 'selected' : '' }}>4D</option>
                        </select>
                ...
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="font-bold text-blue-600 uppercase tracking-wider text-sm">Kredensial Akun Mahasiswa</h3>
                </div>
                ...
                <div class="flex flex-col mb-4">
                    ...
                    {{-- Mengambil nilai username melalui relasi model user --}}
                    <input type="text" name="username" id="username" value="{{ old('username', $mahasiswa->user->username ?? '') }}" readonly
                        class="w-full px-4 py-2 bg-gray-100 border border-gray-200 text-gray-500 rounded-lg outline-none cursor-not-allowed transition text-sm font-mono">
                    @error('username') <p class="text-red-500 text-[10px] mt-1 ml-1">Username/NPM ini sudah terdaftar sebagai akun user lain.</p> @enderror
                </div>
                ...
                <div class="bg-blue-50 text-blue-600 px-4 py-3 rounded-lg text-[11.5px] flex items-center gap-2 mb-4 border border-blue-100">
                    ...
                    <p><strong>Info:</strong> Biarkan kolom password kosong jika tidak ingin mengubah password bawaan mahasiswa.</p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div class="flex flex-col">
                        <label class="text-[11px] font-bold text-gray-700 mb-0.5 ml-1 uppercase">Password Baru</label>
                    ...
                    <div class="flex flex-col">
                        <label class="text-[11px] font-bold text-gray-700 mb-0.5 ml-1 uppercase">Konfirmasi Password Baru</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Listener ketikan real-time sewaktu admin mengedit NPM
